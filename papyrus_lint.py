#!/usr/bin/env python3
"""
VENDORED COPY — the canonical source is desktop-tutorial's
scripts/papyrus_lint.py. This copy exists so papyrus_helpers.py's export()
can self-check its own generated output with zero extra runtime dependency
(stdlib only, works inside Blender's Python). Keep in sync manually when the
canonical version changes — same "known, accepted limitation, not a silent
gap" treatment as check_parity.py's ARTIFACT_PAIRS in the other repo: this
note exists so nobody has to rediscover that two copies exist.
