...
TOOL_DIR = ADDON_ROOT / "tools" / "AssetRipper"
...
def status() -> tuple[bool, str]:
    ...
    if TOOL_DIR.exists() and README_FILE.exists():
        return True, "AssetRipper repo ready"
    if TOOL_DIR.exists():
        return False, "AssetRipper repo found but appears incomplete"
    return False, "AssetRipper repo missing"
...
def download_latest() -> tuple[bool, str]:
    """Download the upstream repo zip to tools/AssetRipper."""

    if TOOL_DIR.exists():
        return True, "AssetRipper directory already exists; skipping download"
    ...
    for url in candidates:
        try:
            ...
            return True, f"Downloaded AssetRipper from {url}"
