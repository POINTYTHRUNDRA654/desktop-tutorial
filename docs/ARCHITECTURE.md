...
- **"Mossy Bridge"** = the **Desktop Bridge** (`BridgeServer.ts`) — screen capture, clipboard, hardware info, Blender add-on channel (port 9999), plus a `/execute` relay to Blender and various file/game/backup routes. Lives inside Runtime Hub (#18), not its own sidebar entry. **Security (2026-08-14):** was unauthenticated with wide-open CORS on a fixed port (21337) — any webpage open in any tab while Mossy ran could reach every endpoint, including a since-removed `type:'shell'` handler that ran arbitrary OS commands via Node's `exec()`. Fixed: every request now requires a locally-generated `X-Mossy-Token` header (timing-safe compared), and the server binds an OS-assigned ephemeral port instead of a fixed one — see `bridgeClient.ts`'s module comment and the GitHub Security Advisory for this repo. The renderer's own calls go through a shared `bridgeFetch()` client (`src/renderer/src/lib/bridgeClient.ts`) that discovers the live port+token via IPC (`get-bridge-connection`) rather than each of the ~30 call sites hand-rolling auth. The legacy standalone Python reimplementation (`mossy_server.py`, previously offered as a downloadable alternative launcher from the Desktop Bridge panel) was retired in the same fix — it had no authentication at all and bound to `0.0.0.0` (all network interfaces), strictly worse than the built-in bridge it duplicated. See "Bridge auth: current state and intended direction" below for what the current token design deliberately doesn't cover yet.
