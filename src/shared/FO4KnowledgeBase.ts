...
endWhile`
  ...
  papyrusThreadingGotchas: {
    utilityWaitInsideEventHandlersQueuesRatherThanDrops: {
      ...
      fix: "Don't hold the script locked in Wait() at all. Have the event handler do its immediate work and return right away, then hand the reset/cooldown off to StartTimer(float afInterval, int aiTimerID) / Event OnTimer(int aiTimerID) combined with a GoToState(\"Busy\") pattern where the busy state deliberately does NOT implement the triggering event — so re-entry during the busy window is a real no-op (found and ignored) instead of a queued replay. StartTimer registrations are stored in the save and correctly reschedule after a save/load, which also structurally eliminates the stuck-forever risk above (a suspended Wait() stack does not have that guarantee). Use CancelTimer(aiTimerID) in any force-reset path so a stale timer can't fire after the state's already been reset. Add an OnReset() handler (in every state the object can be in) that force-clears state back to ready as a second, independent safety net — cheap insurance against the engine resetting the reference while it happens to be mid-cycle.",
      correctedFromAnEarlierWrongFix: "This entry originally recommended RegisterForSingleUpdate()/RegisterForSingleUpdateGameTime() and Event OnUpdate() as the fix. Those do NOT exist anywhere in Fallout 4's Papyrus API — confirmed 2026-08-19 by reading the real, complete ScriptObject.psc (FO4's actual base script source, both the game's installed copy and via PapyrusCompiler.exe itself): no 'SingleUpdate' anywhere in the vanilla source tree, and a real compile of the 'fixed' script using that API failed with 'RegisterForSingleUpdate is not a function or does not exist.' That's real, commonly-used Skyrim Papyrus functionality that simply doesn't carry over to FO4 — a fact a thorough manual code review (twice, by two different reviewers) did not catch, because neither review ran a real compiler. The corrected fix above (StartTimer/OnTimer) was verified by actually compiling it with PapyrusCompiler.exe, not just read again more carefully. Lesson worth keeping distinct from the bug this entry otherwise documents: a knowledge-base entry itself can confidently state a wrong fix and pass review right up until it's checked against real ground truth.",
      verifiedAgainst: "Papyrus Threading Notes on the real Creation Kit wiki (falloutck.uesp.net) confirm the queue-not-drop behavior for a locked script instance. The StartTimer/OnTimer fix pattern was verified by actually compiling a real .psc file with PapyrusCompiler.exe (Fallout 4's real standalone compiler, no CK/GUI needed) — the authoritative check, after the RegisterForSingleUpdate version failed that same real compile.",
      alsoAGeneratorBug: "This wasn't only a hand-written-script bug — the Mossy Fallout 4 Blender Add-on's own template generator (papyrus_helpers.py) baked the identical buggy pattern into MULTIPLE templates, not just the one proximity-trap template first reviewed: _t_spore_activator's simpler OnActivate cooldown and _t_magic_effect's recurring damage tick had the same Wait()-queuing/nonexistent-API pattern, found only by a full real-compile sweep of all 20 templates in the generator (2026-08-19), which also turned up several unrelated pre-existing compile bugs in that file (wrong event signatures for OnEquipped/OnUnequipped, GetDisplayName vs the real GetName(), a local variable named the reserved word 'state', a fabricated F4SE.SetINISetting API that doesn't exist in real F4SE) — all fixed and verified by the same real compiler, 20/20 templates now compile clean. Worth remembering as a category, not just this one instance: a template/generator is a single point of failure for every future output — fixing one generated artifact without checking whether it came from a generator, and without actually compiling the result, is only half the fix."
    },
