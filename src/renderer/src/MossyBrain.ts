...
To build F4SE engine-level plugins in 2026: install Visual Studio 2022 Community with "Desktop development with C++" workload (MSVC v143, Windows SDK 10.0.22621+, CMake tools). Install Git, CMake 3.26+, and vcpkg (\`git clone https://github.com/microsoft/vcpkg C:\vcpkg && .\bootstrap-vcpkg.bat && .\vcpkg integrate install\`; set VCPKG_ROOT env var). Clone CommonLibF4: \`git clone https://github.com/Ryan-rsm-McKenzie/CommonLibF4\` — this gives RE:: class headers for BSLightingShaderProperty, BSDecalNode, TESWeather, bhkWorld, etc. Start from the F4SE Plugin Template (Expired6978/F4SEPluginTemplate) which includes a working CMakeLists.txt, vcpkg.json manifest, F4SE entry point, and GitHub Actions CI for OG/NG DLL builds. CMake configure: generator "Visual Studio 17 2022", platform x64, toolchain C:\vcpkg\scripts\buildsystems\vcpkg.cmake, triplet x64-windows-static. Always build Release (not Debug) — Debug builds cause timing-related crashes in-game. Set post-build event to xcopy the compiled .dll to Data/F4SE/Plugins/. Runtime library must be /MT (static CRT). F4SE entry point is F4SE_InitPlugin(const F4SE::LoadInterface*) — register it with F4SE::Init(a_f4se), allocate trampoline (F4SE::AllocTrampoline(1<<10)), set up spdlog file sink to Documents\My Games\Fallout4\F4SE\MyPlugin.log. Verify hook loaded: check for your log line in that file after launching via f4se_loader.exe. Never hard-code hex offsets — use REL::ID with the Fallout 4 Address Library (nikitalita/address_library on GitHub) which maps function IDs to correct offsets for OG (1.10.163) and NG (1.10.980+) versions. FOMOD installer selects correct DLL per game version. Use xbyak or Detours (via vcpkg) for write_call<5> function hooks. Run CLASSIC crash log analyzer (Nexus #56255) to diagnose hook crashes. BSLightingShaderProperty.h and BSDecalNode.h live in CommonLibF4/include/RE/B/.
