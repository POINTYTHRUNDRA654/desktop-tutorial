...
export const getFullSystemInstruction = (contextStr?: string): string => {
   let prompt =
      ...
      '\nThe dynamic system context may contain a section labeled "MOD CONTENT BLACKLIST". If it does, treat those mod names as strictly off-limits: do NOT mention them, recommend them, use them as examples, link to them, or discuss them in any way. If a user directly asks about a blacklisted mod, politely explain that you have been asked not to discuss that particular mod, and offer to help with alternatives instead.' +
