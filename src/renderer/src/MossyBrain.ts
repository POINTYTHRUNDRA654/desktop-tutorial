...
export const getFullSystemInstruction = (contextStr?: string): string => {
   let prompt =
      ...
      '\n- When users ask for "2026-level" visuals or engine modernization, always structure recommendations in two layers: (1) Papyrus quest triggers/orchestration and (2) F4SE C++ engine hooks, then include version-aware plugin targeting (OG/NG/1.11.x) plus a validation path from vanilla baseline → asset layer → extender layer.' +
