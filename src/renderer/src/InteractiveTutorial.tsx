...
export const InteractiveTutorial: React.FC<InteractiveTutorialProps> = ({ onComplete, onSkip, testPipMode }) => {
  ...
  const steps: TutorialStep[] = [
    ...
    }),
    {
      ...
      mossyText:
        "That's the full tour — nice work making it through! You've seen every tool and module this app has to offer.\n\n**What's new since the last major release:**\n• **Background Remover** (Textures & Materials Hub) — local RMBG-2.0 GPU background removal, or a ComfyUI-RMBG backend with permissively-licensed BEN2/InSPyReNet/BEN models.\n• **AI Post-Processing Pipeline** (Textures & Materials Hub) — Layer Effects, Face Detailer, Relight, and Upscale (SUPIR or UltimateSDUpscale), each running through your own local ComfyUI with a one-click custom-node install.\n• **AI Image Studio: Transparency & Inpaint** — a \"Generate with transparency\" toggle for real alpha-channel output, plus a full brush-based mask editor for crop-and-stitch inpainting.\n• **AI Texture Tools Setup Wizard** — runs right after Voice Setup on first install (or when you replay Initial Install from Settings → Tutorial & Onboarding) and walks you through installing all of the above.\n• **Spriggit Plugin Digest** — after first-run setup you can convert your .esp/.esm/.esl files to YAML and feed them directly into my brain so I know your exact plugin data from day one.\n• **Training Data Pipeline** — use the 👍 / 👎 buttons on any of my chat responses to build a personal training dataset that can be exported in Unsloth ShareGPT format.\n• **Texture Enhancer: Gemini / \"Nano Banana\" backend** — AI Detail Synthesis can now call Google's Gemini image models directly (your own API key) for cloud restyle quality, alongside the original local ComfyUI backend.\n• **Automatic background removal** — AI Detail Synthesis has a \"Remove background after generation\" toggle, producing a transparent-alpha cutout in the same pass, for either backend.\n• **One-click SDXL checkpoint + Tile ControlNet download** — no more dead end if the local ComfyUI backend has no checkpoint installed; a \"Lock UV layout with ControlNet Tile\" toggle keeps a texture restyle's structure fixed during generation.\n• **AI Post-Processing Pipeline: Image to 3D** — free, fully local single-image-to-mesh reconstruction (TripoSR), installed the same one-click way as the pipeline's other tools.\n\nFrom here, I'd suggest starting with something simple like a texture swap to get your hands dirty. When you have questions, just open the Chat — I'm always here. The Learning Hub is great for going deeper on specific topics like scripting or quest design. And the Project Hub helps you stay organized as things get more complex.\n\nRemember: I'm not affiliated with any third-party tools — always get them from their official sources. Don't worry about remembering everything at once. Just dive in, experiment, and ask when you get stuck. That's how every good modder gets started. Happy modding!",
      route: '/',
