    ...
    try {
      ...
      const sessionPromise = ai.live.connect({
        ...
        callbacks: {
          onopen: () => {
            try {
              console.log('[LiveContext] onopen: Setting up audio processing');
              setStatus('Online');
              setIsActive(true);
              setMode('listening');
              ...
              proc.onaudioprocess = (evt: any) => {
                try {
                  const data = evt.inputBuffer.getChannelData(0);
                  let sum = 0;
                  for (let i = 0; i < data.length; i++) sum += data[i] * data[i];
                  const vol = Math.sqrt(sum / data.length) * 500;
                  setVolume(vol);
                  
                  const blob = createBlob(data);
                  const sess = sessionRef.current;
                  if (sess) sess.then((s: any) => { if (s) s.sendRealtimeInput({ media: blob }); });
                } catch (e) {
                  console.error('[LiveContext] audio error:', e);
                }
              };
              
              src.connect(proc);
              proc.connect(ctx.destination);
              console.log('[LiveContext] Audio setup complete');
            } catch (err: any) {
              console.error('[LiveContext] onopen failed:', err?.message);
              disconnect();
            }
          },
