...
const ExternalToolsSettings: React.FC<ExternalToolsSettingsProps> = ({ embedded = false }) => {
  ...
  const handleCheckPyTorch = async () => {
    ...
    try {
      ...
      if (result?.available) {
        let status = `✅ PyTorch ${result.version || ''} found`;
        if (result.computeMode === 'CUDA') status += ' (GPU-accelerated with CUDA)';
        else if (result.computeMode === 'CPU') status += ' (CPU-only)';
        status += ` at ${result.path || 'system Python'}`;
        setPytorchCheckStatus(status);
        if (result.path && !draft.pytorchPath) {
          handleChange('pytorchPath', result.path);
        }
      } else if (result?.cudaIssue && result?.troubleshooting) {
        // Display CUDA-specific troubleshooting guide
        const troubleshootingText = result.troubleshooting.join('\n');
        setPytorchCheckStatus(`⚠️ ${result?.error || 'PyTorch not available'}\n\n${troubleshootingText}`);
      } else {
  ...
  /** Auto-install PyTorch with CUDA diagnostics and troubleshooting. */
    ...
    <div className={containerClassName}>
      ...
      <div className={contentClassName}>
        ...
        <div className="mb-6 grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="p-4 bg-slate-900 border border-slate-800 rounded-lg">
            <div className="text-sm font-bold text-white mb-2">TripoSG (Local Repo)</div>
            <div className="text-xs text-slate-300 mb-3">
              Adds the local TripoSG repo to Knowledge Search so you can index image-to-3D docs and Fallout 4 downstream mesh workflow guidance.
            </div>
            <div className="flex flex-wrap gap-2">
              <button
                onClick={() => addCosmosKnowledgeRoot('external/VAST-AI-Research/TripoSG', 'TripoSG')}
                className="px-3 py-2 bg-emerald-700 hover:bg-emerald-600 border border-emerald-500 rounded text-[11px] font-bold"
              >
                Add to Knowledge Search Roots
              </button>
            ...
            <div className="mt-2 text-[10px] text-slate-500">
              Default path: external/VAST-AI-Research/TripoSG
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
            ...
            {pytorchCheckStatus && (
              <div className={`mt-2 text-[11px] font-mono px-2 py-1 rounded break-all ${pytorchCheckStatus.startsWith('✅') ? 'bg-emerald-900/40 text-emerald-300' :
