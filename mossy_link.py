...
def _push_context_to_mossy(scene, depsgraph):
    ...
    try:
        ...
        _th.Thread(target=_do_push, daemon=True).start()
    except Exception:
        pass
...
_FO4_SYSTEM_CONTEXT = (
    ...
    "\n\nTypical FO4 mod workflow stages:\n"
    "1. Mesh prep: validate, triangulate, UV, materials\n"
    "2. Collision: UCX_ objects, Havok layers (L_STATIC=1, L_CLUTTER=6)\n"
    "3. NIF export: PyNifly preferred, native writer fallback\n"
    "4. BGSM/texture: DDS conversion (diffuse BC7, normal BC5, spec BC4), path setup\n"
    "5. LOD: LOD1/2/3 NIFs, xLODGen for world-space LOD\n"
    "6. ESP: Creation Kit static/furniture/flora record\n"
    "7. BA2 packaging and FOMOD installer\n"
    "8. In-game testing: load order, missing mesh debug, collision debug mode\n"
)
...
def register() -> None:
    ...
    # Register the real-time context-push depsgraph handler
    try:
        import bpy as _bpy
        if _push_context_to_mossy not in _bpy.app.handlers.depsgraph_update_post:
            _bpy.app.handlers.depsgraph_update_post.append(_push_context_to_mossy)
    except Exception as exc:
        print(f"[Mossy Link] Could not register depsgraph handler: {exc}")
...
def unregister() -> None:
    ...
    try:
        ...
        pass
    # Remove the real-time context-push depsgraph handler
    try:
        import bpy as _bpy
        if _push_context_to_mossy in _bpy.app.handlers.depsgraph_update_post:
            _bpy.app.handlers.depsgraph_update_post.remove(_push_context_to_mossy)
    except Exception:
        pass
    try:
        stop_server()
