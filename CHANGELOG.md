...
### Local Fallback — Lemonade + Qwen3-4B, Real Tool-Calling (2026-08-24)

**New: Brain B's local fallback can now actually use tools.** When the primary Groq/cloud path fails or is unreachable, the local fallback now calls Lemonade Server (if installed and running) running Qwen3-4B-Instruct-2507 over its OpenAI-compatible endpoint — the first local fallback in this app able to return real, structured tool calls instead of plain text. Chosen over Lemonade's Ollama- and Anthropic-compatible dialects (Lemonade genuinely supports all three unchanged) because the OpenAI dialect needed the least new client code: `src/electron/ml/openaiCompat.ts` already existed as a working, Electron-free OpenAI-compat client, and its tool-call argument shape already matches Groq's own convention (`src/backend/routes/chat.ts`'s `BackendToolCall`) exactly, so no new parsing logic was needed.

**Real, live-verified 2026-08-24** against Lemonade Server v11.7.0: the same 3-test protocol already proven against Groq/qwen3.6-27b (conversational/auto with no false tool call, forced single tool call, forced multi/parallel tool call) passed cleanly twice — once directly against Lemonade, and again end-to-end through the actual shipped fallback modules (`src/electron/ml/lemonade.ts` and the extended `openaiCompat.ts`, unmodified, driven exactly the way `LocalAIEngine.ts`'s `attemptLocalFallback()` calls them) — across 13+ repeated runs including a 4-tool stress test, zero HTTP 500s, zero backend crashes. Separately confirmed GitHub issue #1562 ("llama.cpp backend crashes on parallel tool calls from small models") did not reproduce here and is still open upstream, not fixed — an already-closed, unrelated grammar/JSON-schema bug (#2691) was initially mistaken for it before checking the issue tracker directly.

**Honest degradation:** a fast health probe against Lemonade's real `/health` endpoint (1.5s cap, measured ~26ms against a closed port) runs before any chat call, so a genuinely-not-running Lemonade fails immediately with a real error instead of the request waiting out a full chat-completion timeout on top of the Groq failure that already happened. On a tool-calling turn specifically, if Lemonade is down there is nowhere else for the request to go — the older generic local-provider path (Ollama/LM Studio/Cosmos) is plain single-prompt text with no tool support at all — so it now says so plainly ("Mossy's local fallback isn't available right now — Lemonade Server ... isn't running or isn't reachable") rather than silently returning prose that never does what was asked, or hanging. Plain conversational fallback turns (no tools requested) still try Lemonade first, then fall through to the existing Ollama/Cosmos/LM Studio path unchanged as a second-tier fallback.

**What's tested vs. not yet covered:** the request/response modules were verified end-to-end against the real running server by importing the actual unmodified production code, not a reimplementation. The renderer-side wiring in `LocalAIEngine.ts`'s `attemptLocalFallback()` — which assembles the real system prompt and conversation history into the messages sent to Lemonade, and is only reachable through Electron's IPC bridge — was reviewed and type-checked but **not** re-exercised through a full running Electron app this session; that's the one gap between "the plumbing is proven correct" (confirmed) and "a real failed Groq call in the live app visibly recovers through Lemonade" (not yet independently re-confirmed that way). Also not yet covered: Lemonade isn't a first-class configured provider in Settings the way Ollama/LM Studio are — no UI exists yet for a `lemonadeBaseUrl`/`lemonadeModel` override, though the backend already honors one if a future settings field sets it. Also found, while investigating the wiring: `IPC_CHANNELS` was independently duplicated in three places (`src/electron/types.ts`, `src/shared/types.ts`, and inlined directly in `preload.ts`, which doesn't import from either) — all three needed the new channel names added by hand to avoid a silent divergence matching this project's known forked-file failure pattern.
